...
from .pgseg_seg import PGSegInference
...
def build_seg_inference(model, dataset, text_transform, config):
    ...
    with_bg = dataset.CLASSES[0] == 'background'  # Test whether has background
    if with_bg:
        classnames = dataset.CLASSES[1:]
    else:
        classnames = dataset.CLASSES
    text_tokens = build_dataset_class_tokens(text_transform, config.template, classnames) #class_num*1*max_length
    text_embedding = model.build_text_embedding(text_tokens) # class_num*embedding_dim(256)
    kwargs = dict(with_bg=with_bg)
    if hasattr(cfg, 'test_cfg'):
        kwargs['test_cfg'] = cfg.test_cfg
    seg_model = PGSegInference(model, text_embedding, **kwargs)
