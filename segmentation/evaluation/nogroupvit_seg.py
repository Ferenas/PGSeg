...
def resize_attn_map(attentions, h, w, align_corners=False):
    ...
    # if stage == 1:
    #     o_img = F.interpolate(o_img,(h_featmap,w_featmap))[0].permute(1,2,0).cpu().numpy()
    #
    #     # from skimage.segmentation import slic
    #     # sc = np.repeat(np.expand_dims(slic(o_img),axis=-1),axis=-1,repeats=3)
    #     # aff_sc = rw_affinity(sc.astype(np.float64)/np.max(sc),radius=1)
    #     # W_sc = np.array(aff_sc.todense().astype(np.float32))
    #     # W_sc= torch.from_numpy(W_sc).unsqueeze(0).cuda()
    #
    #     affinity_o = rw_affinity(o_img/ (255. * 255.))
    #     W_color = np.array(affinity_o.todense().astype(np.float32))
    #     W_color = torch.from_numpy(W_color).unsqueeze(0).cuda()
    #
    #     aff = W_color
...
class NoGroupViTSegInference(EncoderDecoder):
    ...
    def __init__(self, model, text_embedding, with_bg, test_cfg=dict(mode='whole', bg_thresh=.95)):
        ...
        self.center_np1, self.center_np2 = [], []
        self.p1,self.p2 = [], []
        self.l1,self.l2 = [], []
        if self.with_bg:
    ...
    def get_attn_maps(self, img, return_onehot=False, rescale=False):
        ...
        #         'return_attn' : True,
        # }
        # flops,params = profile(self.model.img_encoder,inputs=(img,),extra_ins=inp)
        # flops,params = clever_format([flops,params])
        results = self.model.img_encoder(img, return_attn=True, as_dict=True)  # Group_vit encoder for image
        import numpy as np
        attn_dicts=results['attn_dicts']

        # input1 = attn_dicts[0]['ori_input'].cpu().numpy()
        # input2 = attn_dicts[1]['ori_input'].cpu().numpy()
        # label_1 = torch.argmax(attn_dicts[0]['hard'][0,0,:,:],dim=0).cpu().numpy()
        # label_2 = torch.argmax(attn_dicts[1]['hard'][0,0,:,:],dim=0).cpu().numpy()
        #
        #
        #
        # center1 = attn_dicts[0]['ori_center'].cpu().numpy()
        # center2 = attn_dicts[1]['ori_center'].cpu().numpy()


        # self.center_np1.append(center1)   #center1
        # self.center_np2.append(label_2)
        #
        # self.p1.append(input1)
        # self.p2.append(input2)
        # self.l1.append(label_1)
        # self.l2.append(label_2)
        # np.save('newc1_tnpr.npy',self.center_np1)
        # np.save('newc2_tnpr.npy',self.center_np2)

        # np.save('newp1_inpr.npy',self.p1)
        # np.save('newp2_inpr.npy',self.p2)
        # np.save('newl1_inpr.npy',self.l1)
        # np.save('newl2_inpr.npy',self.l2)
        attn_maps = []
        with torch.no_grad():
            ...
            for idx, attn_dict in enumerate(results['attn_dicts']):
                ...
                if prev_attn_masks is None:
                    ...
                    # attn_masks = torch.matmul(attn_pixel2,attn_masks[0]).unsqueeze(0)
                    prev_attn_masks = prev_attn_masks @ attn_masks
                # [B, nH, HxW, G] -> [B, nH, H, W, G]
                attn_maps.append(resize_attn_map(prev_attn_masks, *img.shape[-2:]))

        # attention maps = [64*448*448,8*448*448]
        ...
        return attn_maps  ##attention maps = [64*448*448,8*448*448]  [attn_maps[0]]
    ...
    def encode_decode(self, img, img_metas):
        ...
        ori_img = tensor2imgs(img, **img_metas[0]['img_norm_cfg'])[0]

        attn_map = self.get_attn_maps(img, rescale=True)  # [448*448*8]
        attn_map = attn_map[-1][0]

        img_outs = self.model.encode_image(img, return_feat=True,
                                           as_dict=True)  # [image_feature 1*8*256 and with pooling 1*1*256]
        # [B, L, C] -> [L, C]
        ...
        # [G, N]
        group_affinity_mat = (grouped_img_tokens @ text_tokens.T) * logit_scale  # 8*num_clases
        pre_group_affinity_mat = F.softmax(group_affinity_mat, dim=-1)

        avg_affinity_mat = (img_avg_feat @ text_tokens.T) * logit_scale
        avg_affinity_mat = F.softmax(avg_affinity_mat, dim=-1)  # 1*num_classes
        affinity_mask = torch.zeros_like(avg_affinity_mat)
        avg_affinity_topk = avg_affinity_mat.topk(dim=-1, k=min(5, num_fg_classes))  # only do softmax on top 5 classes
        ...
        group_affinity_mat = F.softmax(group_affinity_mat, dim=-1)  # 8*20
        ...
        o_final = rearrange(onehot_attn_map @ group_affinity_mat, 'h w c -> c h w')
        # o_atten = F.interpolate(o_atten.unsqueeze(0),(r_h,r_w))
        # for _ in range(iter):
        #     o_attn = torch.matmul(o_atten.flatten(start_dim=2),lam*W_color).view(n,-1,r_h,r_w)
        # o_final = F.interpolate(o_attn,(h,w))[0] / (pow(lam,2))

        # o_final = o_atten
        # o_final = o_atten / (F.adaptive_max_pool2d(o_atten,(1,1))+1e-6)
        # o_final /= F.adaptive_max_pool2d(o_final,(1,1)) + 1e-6

        # o_atten = F.interpolate(o_atten.unsqueeze(0),(r_h,r_w))
        # for _ in range(iter):
        #     o_attn = torch.matmul(o_atten.flatten(start_dim=2),lam*W_color).view(n,-1,r_h,r_w)

        # o_final = F.interpolate(o_final.unsqueeze(0), (r_h, r_w))
        # o_final = torch.matmul(o_final.flatten(start_dim=2),attn_pixel).view(n,-1,r_h,r_w)
        # o_final = F.interpolate(o_final, (h, w))[0]

        pred_logits[class_offset:] = o_final  # o_attn_map[0] #o_final
        if self.with_bg:
            bg_thresh = min(self.bg_thresh, group_affinity_mat.max().item())
            pred_logits[0, o_final.max(dim=0).values < bg_thresh] = 1  # o_attn_map[0].max(dim=0).values
    ...
    def show_result(self, img_show, img_tensor, result, out_file, vis_mode='input'):
        ...
        if vis_mode == 'input':
            ...
            for layer_idx, attn_map in enumerate(attn_map_list):
                ...
                if vis_mode == 'final_group':
                    atten_out = out_file.replace('final_group', 'final_group_atten')
                    mmcv.mkdir_or_exist(osp.dirname(atten_out))
                    np.save(atten_out.replace('.jpg', '.npy'), group_result)
