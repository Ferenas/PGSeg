...
class MultiLabelContrastive(nn.Module):
    ...
    def encode_image(self, image, *, return_feat=False, as_dict=False,return_pgloss=False,text=None):
        outs = Result(as_dict)

        img_outs = self.img_encoder(image, return_feat=return_feat, as_dict=True,return_attn=True,return_pgloss=return_pgloss,text=text)

        outs.append(self.img_projector(img_outs['x']), 'image_x')
        outs.append(img_outs['attn_dicts'],'cross_attn')
        # outs.append(img_outs['attn_o_list'][0],'attn_o_list')
        if return_pgloss:
            outs.append(img_outs['loss_pg'],'loss_pg')
        if return_feat:
    ...
    def encode_text(self, text, *, as_dict=False):
        ...
        text_x = self.text_projector(x[0])

        outs.append(text_x, 'text_x')
        outs.append(x[1],'text_mu')
        if squeeze_dim:
    ...
    def forward_train(self, image, text):
        ...
        text_mu = text_outs['text_mu']

        image_outs = self.encode_image(image, as_dict=True,return_pgloss=True,text=text_mu)
        # [B, C]
        ...
        losses_dict['loss_pg'] = image_outs['loss_pg']
    ...
    def build_text_embedding(self, text):
        ...
        # [N, T, C]
        text_tokens = rearrange(text_tokens[0], '(n t) c -> n t c', n=num_classes, t=num_templates)
        # [N, C]
